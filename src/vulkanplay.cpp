...
VkCommandBuffer VulkanPlayApp::beginSingleTimeCommands() {
	VkCommandBufferAllocateInfo allocInfo{};
	allocInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
	allocInfo.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY;
	allocInfo.commandPool = commandPool;
...
void VulkanPlayApp::endSingleTimeCommands(VkCommandBuffer commandBuffer) {
...
void VulkanPlayApp::createDescriptorPool() {
	...
	ERROR_CHECK(vkCreateDescriptorPool(device, &poolInfo, nullptr,
									   &descriptorPool) != VK_SUCCESS,
				"Failed to create descriptor pool!");
}

void VulkanPlayApp::createDescriptorSets() {
	...
	ERROR_CHECK(vkAllocateDescriptorSets(device, &allocInfo,
...
void VulkanPlayApp::createImage(uint32_t width, uint32_t height,
								uint32_t mipLevels,
	...
	ERROR_CHECK(vkAllocateMemory(device, &allocInfo, nullptr, &imageMemory) !=
					VK_SUCCESS,
...
void VulkanPlayApp::createTextureImage() {
	...
	generateMipmaps(textureImage, VK_FORMAT_B8G8R8A8_SRGB, texWidth, texHeight,
					mipLevels);
}
...
void VulkanPlayApp::copyBufferToImage(VkBuffer buffer, VkImage image,
...
VkImageView VulkanPlayApp::createImageView(VkImage image, VkFormat format,
...
void VulkanPlayApp::createTextureSampler() {
	...
	ERROR_CHECK(vkCreateSampler(device, &samplerInfo, nullptr,
								&textureSampler) != VK_SUCCESS,
				"Failed to create texture sampler!");
}

void VulkanPlayApp::createDepthResources() {
...
bool VulkanPlayApp::hasStencilComponent(VkFormat format) {
	return format == VK_FORMAT_D32_SFLOAT_S8_UINT ||
		   format == VK_FORMAT_D24_UNORM_S8_UINT;
}

VkFormat VulkanPlayApp::findDepthFormat() {
...
VkFormat VulkanPlayApp::findSupportedFormat(
...
void VulkanPlayApp::generateMipmaps(VkImage image, VkFormat imageFormat,
...
VkSampleCountFlagBits VulkanPlayApp::getMaxUsableSampleCount() {
...
void VulkanPlayApp::createColorResources() {
